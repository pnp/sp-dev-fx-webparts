...
type getUserProfileFunc = ( currentUser: string,
 ...
 type getUsersFunc = (department: string,pagesize?:number
  ) => Promise<SearchResults>; 

type returnProfileData =  {  currentUserProfile :IPersonProperties} ;

export const useGetUserProperties  =  (context:WebPartContext):  { getUserProfile:getUserProfileFunc , getUsers: getUsersFunc }  => {
  ...
  const getUserProfile = React.useCallback(
    async (
      ...
      const cacheCurrentUser:IPersonProperties = await get(`${loginName}__staffDirectory__`);
     ...
     const _department =   filter(currentUserProfile?.UserProfileProperties,{"Key": "Department"})[0].Value ?? '';
    
     const r = await  getUsers(_department ? `Department: ${_department}`: '*');
     console.log(r);
      } else {
  ...
  const getUsers = async  (search:string,pageSize?:number ):Promise<SearchResults> => {
  ...
  for (const r of wPrimarySearchResults) {
    wUsers.push({
     ...
     pictureBase64: r["PictureURL"] ? `/_layouts/15/userphoto.aspx?size=M&accountname=${r["WorkEmail"]}` : undefined, 
