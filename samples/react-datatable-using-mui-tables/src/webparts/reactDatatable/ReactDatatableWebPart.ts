...
export default class ReactDatatableWebPart extends BaseClientSideWebPart<IReactDatatableWebPartProps> {
  ...
  public async getSelectedListFields() {
    if (this.properties.list) {
      ...
      this.properties.fieldDetails = allFields.map(field => ({ key: field.InternalName, text: field.Title, fieldType: field['odata.type'], allowMultipleValues: field['AllowMultipleValues'] ? field['AllowMultipleValues'] : false }));
