...
const RichTextEditor: React.FC<IRichTextEditorProps> = ({ value, onChange, disabled, context }) => {
  ...
  // Track if we're currently updating to prevent cursor jumping
  ...
  // Only update editor content when value changes externally (not from our own edits)
  useEffect(() => {
    if (!isSourceMode && editorRef.current && !isUpdatingRef.current) {
  ...
  // Use callback to debounce onChange calls
  ...
  const insertTable = (): void => {
    ...
    let tableHtml =
      '<table style="border-collapse: collapse; width: 100%; margin: 10px 0; border: 1px solid #c8c6c4;">';
    for (let i = 0; i < rows; i++) {
      tableHtml += '<tr>';
      for (let j = 0; j < cols; j++) {
        const tag = i === 0 ? 'th' : 'td';
        const style =
          'border: 1px solid #c8c6c4; padding: 8px; ' +
          (tag === 'th' ? 'background-color: #f3f2f1; font-weight: 600;' : '');
  ...
  const insertImage = async (): Promise<void> => {
    ...
    try {
      ...
      if (selectedFile && context && imageService.current) {
        ...
        try {
          ...
          if (uploadResult.success && uploadResult.absoluteUrl) {
            ...
            setUploadMessage({
              text: 'Image uploaded successfully to SharePoint!',
      ...
      const imgHtml = `<img src="${sanitizeHtml(
        finalImageUrl
      )}" style="max-width: 100%; height: auto; margin: 10px 0; border-radius: 4px;" alt="Inserted image" />`;
  ...
  return (
    <div className={styles.richTextEditor}>
      <div className={styles.toolbar}>
        <Stack horizontal tokens={{ childrenGap: 5 }} wrap>
          <Dropdown
            ...
            onChange={(_e, option) => option && applyHeading(option.key as string)}
          ...
          <TooltipHost content="Text Color">
            <IconButton
              ...
              onClick={() => execCommand('foreColor', '#0078d4')}
              disabled={disabled || isSourceMode}
          ...
          <TooltipHost content="Increase Indent">
            <IconButton
              iconProps={{ iconName: 'IncreaseIndent' }}
          ...
          <TooltipHost content="Insert Horizontal Line">
            <IconButton
              ...
              disabled={disabled || isSourceMode}
              ariaLabel="Insert Horizontal Line"
            />
          </TooltipHost>
      ...
      {/* Image and Table dialogs remain the same */}
      <Dialog
        ...
        <div className={styles.dialogContent}>
          ...
          {context && (
            <div className={styles.uploadSection}>
              <Label
                styles={{
                  root: { fontWeight: 600, marginBottom: 10, fontSize: 15 }
                }}
              >
          ...
          <Stack tokens={{ childrenGap: 12 }}>
            <Label
              styles={{
                root: { fontWeight: 600, fontSize: 15 }
              }}
            >
              🔗 Enter Image URL
            </Label>
              ...
              <div className={styles.imagePreview}>
                ...
                <img
                  ...
                  onError={e => {
                    ...
                    setUploadMessage({
                      text: 'Failed to load image from URL',
                      type: MessageBarType.error
                    });
                  }}
                />
        ...
        <DialogFooter>
          {isUploading ? (
            ...
            <>
              ...
              />
              <DefaultButton
                onClick={handleImageDialogDismiss}
                text="Cancel"
        ...
        <Stack tokens={{ childrenGap: 15 }}>
          <TextField
            ...
            onChange={(_e, newValue) => {
              const num = parseInt(newValue || '3', 10);
              const clamped = Math.min(Math.max(1, num || 3), 20);
              setTableRows(clamped.toString());
            }}
            min={1}
            max={20}
          />
          <TextField
            ...
            onChange={(_e, newValue) => {
              const num = parseInt(newValue || '3', 10);
              const clamped = Math.min(Math.max(1, num || 3), 10);
              setTableCols(clamped.toString());
            }}
            min={1}
            max={10}
          />
          <MessageBar messageBarType={MessageBarType.info}>
            Table will be created with {tableRows} rows and {tableCols} columns. The first row will be
            headers.
          </MessageBar>
        </Stack>
        <DialogFooter>
