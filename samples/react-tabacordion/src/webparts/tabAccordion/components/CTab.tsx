...
const dragHandleIcon: IIconProps = { iconName: 'GripperBarHorizontal' };
...
export default class CTab extends React.Component<ICTabProps, ICTabState> {
  ...
  public async componentDidMount(): Promise<void> {
    // Show TinyMCE initialization message
    ...
    // Load user data if needed for audience targeting
  ...
  public render(): React.ReactElement<ICTabProps> {
    ...
    const { selectedKey, isLoading, isImporting } = this.state;
    const customStyles = this.getCustomStyles();
    
    // If loading or importing, show spinner
    if (isLoading || isImporting) {
      return (
        <div className={styles.loadingContainer}>
          <Spinner 
            size={SpinnerSize.large} 
            label={isImporting ? "Importing tabs..." : "Loading..."} 
          />
        </div>
      );
    }
              ...
              <div className={styles.tabs}>
                <div className={styles.tablinks} style={customStyles.tablinksStyle}>
                  {tabs.map((tab: any, index: number) => (
                    <div key={index} className={styles.tabContainer} style={{ position: 'relative' }}>
                      <button
                      ...
                      {selectedKey === index.toString() && (
                        <div 
                          ...
                          style={{
                            ...
                            right: 0,
                            height: 2,
                            ...customStyles.activeIndicatorStyle
                          }}
                        />
                      )}
                    </div>
                ...
                {tabs.map((tab: any, index: number) => (
                  <div
                    ...
                    style={{ display: selectedKey === index.toString() ? 'block' : 'none' }}
