...
*/
(function(){window.CKEDITOR&&window.CKEDITOR.dom||(window.CKEDITOR||(window.CKEDITOR=function(){var a=/(^|.*[\\\/])ckeditor\.js(?:\?.*|;.*)?$/i,f={timestamp:"GBUD",version:"4.6.1 (Standard)",revision:"77e20f4",rnd:Math.floor(900*Math.random())+100,_:{pending:[],basePathSrcPattern:a},status:"unloaded",basePath:function(){var b=window.CKEDITOR_BASEPATH||"";if(!b)for(var c=document.getElementsByTagName("script"),f=0;f<c.length;f++){var d=c[f].src.match(a);if(d){b=d[1];break}}-1==b.indexOf(":/")&&"//"!=
...
else if(1==c.rangeCount){var c=c.getRangeAt(0),d=c.startContainer;d==c.endContainer&&1==d.nodeType&&1==c.endOffset-c.startOffset&&B[d.childNodes[c.startOffset].nodeName.toLowerCase()]&&(b=CKEDITOR.SELECTION_ELEMENT)}return a.type=b},getRanges:function(){var a=n?function(){function a(b){return(new CKEDITOR.dom.node(b)).getIndex()}var b=function(b,c){b=b.duplicate();b.collapse(c);var d=b.parentElement();if(!d.hasChildNodes())return{container:d,offset:0};for(var e=d.children,g,f,h=b.duplicate(),k=0,
l=e.length-1,m=-1,n,p;k<=l;)if(m=Math.floor((k+l)/2),g=e[m],h.moveToElementText(g),n=h.compareEndPoints("StartToStart",b),0<n)l=m-1;else if(0>n)k=m+1;else return{container:d,offset:a(g)};if(-1==m||m==e.length-1&&0>n){h.moveToElementText(d);h.setEndPoint("StartToStart",b);h=h.text.replace(/(\r\n|\r)/g,"\n").length;e=d.childNodes;if(!h)return g=e[e.length-1],g.nodeType!=CKEDITOR.NODE_TEXT?{container:d,offset:e.length}:{container:g,offset:g.nodeValue.length};for(d=e.length;0<h&&0<d;)f=e[--d],f.nodeType==
CKEDITOR.NODE_TEXT&&(p=f,h-=f.nodeValue.length);return{container:p,offset:-h}}h.collapse(0<n?!0:!1);h.setEndPoint(0<n?"StartToStart":"EndToStart",b);h=h.text.replace(/(\r\n|\r)/g,"\n").length;if(!h)return{container:d,offset:a(g)+(0<n?0:1)};for(;0<h;)try{f=g[0<n?"previousSibling":"nextSibling"],f.nodeType==CKEDITOR.NODE_TEXT&&(h-=f.nodeValue.length,p=f),g=f}catch(r){return{container:d,offset:a(g)}}return{container:p,offset:0<n?-h:p.nodeValue.length+h}};return function(){var a=this.getNative(),c=a&&
...
f=void 0}if(f)this.fake(f);else{if(n){k=CKEDITOR.dom.walker.whitespaces(!0);h=/\ufeff|\u00a0/;b={table:1,tbody:1,tr:1};1<a.length&&(f=a[a.length-1],a[0].setEnd(f.endContainer,f.endOffset));f=a[0];a=f.collapsed;var p,r,v;if((c=f.getEnclosedNode())&&c.type==CKEDITOR.NODE_ELEMENT&&c.getName()in B&&(!c.is("a")||!c.getText()))try{v=c.$.createControlRange();v.addElement(c.$);v.select();return}catch(q){}if(f.startContainer.type==CKEDITOR.NODE_ELEMENT&&f.startContainer.getName()in b||f.endContainer.type==
CKEDITOR.NODE_ELEMENT&&f.endContainer.getName()in b)f.shrink(CKEDITOR.NODE_ELEMENT,!0),a=f.collapsed;v=f.createBookmark();b=v.startNode;a||(g=v.endNode);v=f.document.$.body.createTextRange();v.moveToElementText(b.$);v.moveStart("character",1);g?(h=f.document.$.body.createTextRange(),h.moveToElementText(g.$),v.setEndPoint("EndToEnd",h),v.moveEnd("character",-1)):(p=b.getNext(k),r=b.hasAscendant("pre"),p=!(p&&p.getText&&p.getText().match(h))&&(r||!b.hasPrevious()||b.getPrevious().is&&b.getPrevious().is("br")),
r=f.document.createElement("span"),r.setHtml("\x26#65279;"),r.insertBefore(b),p&&f.document.createText("﻿").insertBefore(b));f.setStartBefore(b);b.remove();a?(p?(v.moveStart("character",-1),v.select(),f.document.$.selection.clear()):v.select(),f.moveToPosition(r,CKEDITOR.POSITION_BEFORE_START),r.remove()):(f.setEndBefore(g),g.remove(),v.select())}else{g=this.getNative();if(!g)return;this.removeAllRanges();for(v=0;v<a.length;v++){if(v<a.length-1&&(p=a[v],r=a[v+1],h=p.clone(),h.setStart(p.endContainer,
p.endOffset),h.setEnd(r.startContainer,r.startOffset),!h.collapsed&&(h.shrink(CKEDITOR.NODE_ELEMENT,!0),f=h.getCommonAncestor(),h=h.getEnclosedNode(),f.isReadOnly()||h&&h.isReadOnly()))){r.setStart(p.startContainer,p.startOffset);a.splice(v--,1);continue}f=a[v];r=this.document.$.createRange();f.collapsed&&CKEDITOR.env.webkit&&e(f)&&(h=m(this.root),f.insertNode(h),(p=h.getNext())&&!h.getPrevious()&&p.type==CKEDITOR.NODE_ELEMENT&&"br"==p.getName()?(l(this.root),f.moveToPosition(p,CKEDITOR.POSITION_BEFORE_START)):
f.moveToPosition(h,CKEDITOR.POSITION_AFTER_END));r.setStart(f.startContainer.$,f.startOffset);try{r.setEnd(f.endContainer.$,f.endOffset)}catch(E){if(0<=E.toString().indexOf("NS_ERROR_ILLEGAL_VALUE"))f.collapse(1),r.setEnd(f.endContainer.$,f.endOffset);else throw E;}g.addRange(r)}}this.reset();this.root.fire("selectionchange")}}},fake:function(a,b){var c=this.root.editor;void 0===b&&a.hasAttribute("aria-label")&&(b=a.getAttribute("aria-label"));this.reset();h(c,b);var d=this._.cache,e=new CKEDITOR.dom.range(this.root);
...
"false"==e?c=a:"true"==e&&(d=1)}return c}function f(a,b,c,d){return(a.getPosition(b)|d)==d&&(!c.childRule||c.childRule(a))}function b(c){var d=c.document;if(c.collapsed)d=p(this,d),c.insertNode(d),c.moveToPosition(d,CKEDITOR.POSITION_BEFORE_END);else{var g=this.element,h=this._.definition,k,l=h.ignoreReadonly,m=l||h.includeReadonly;null==m&&(m=c.root.getCustomData("cke_includeReadonly"));var n=CKEDITOR.dtd[g];n||(k=!0,n=CKEDITOR.dtd.span);c.enlarge(CKEDITOR.ENLARGE_INLINE,1);c.trim();var r=c.createBookmark(),
...
D=C&&new CKEDITOR.dom.range(D.getDocument());ea<C;++ea){var A=x[ea],ha=CKEDITOR.filter.instances[A.data("cke-filter")];if(ha?ha.check(this):1)D.selectNodeContents(A),b.call(this,D)}x=B?!n[B]||y?0:w&&!m?0:f(t,u,h,K):1;if(x)if(A=t.getParent(),x=h,C=g,ea=k,!A||!(A.getDtd()||CKEDITOR.dtd.span)[C]&&!ea||x.parentRule&&!x.parentRule(A))l=!0;else{if(z||B&&CKEDITOR.dtd.$removeEmpty[B]&&(t.getPosition(u)|K)!=K||(z=c.clone(),z.setStartBefore(t)),B=t.type,B==CKEDITOR.NODE_TEXT||w||B==CKEDITOR.NODE_ELEMENT&&!t.getChildCount()){for(var B=
t,H;(l=!B.getNext(I))&&(H=B.getParent(),n[H.getName()])&&f(H,q,h,J);)B=H;z.setEndAfter(B)}}else l=!0;t=t.getNextSourceNode(y||w)}if(l&&z&&!z.collapsed){for(var l=p(this,d),w=l.hasAttributes(),y=z.getCommonAncestor(),B={},x={},A={},C={},fa,W,F;l&&y;){if(y.getName()==g){for(fa in h.attributes)!C[fa]&&(F=y.getAttribute(W))&&(l.getAttribute(fa)==F?x[fa]=1:C[fa]=1);for(W in h.styles)!A[W]&&(F=y.getStyle(W))&&(l.getStyle(W)==F?B[W]=1:A[W]=1)}y=y.getParent()}for(fa in x)l.removeAttribute(fa);for(W in B)l.removeStyle(W);
...
a.collapsed&&(a.checkBoundaryOfElement(h,CKEDITOR.END)||(k=a.checkBoundaryOfElement(h,CKEDITOR.START)))?(g=h,g.match=k?"start":"end"):(h.mergeSiblings(),h.is(this.element)?q.call(this,h):u(h,t(this)[h.getName()]))}if(g){h=d;for(f=0;;f++){k=e.elements[f];if(k.equals(g))break;else if(k.match)continue;else k=k.clone();k.append(h);h=k}h["start"==g.match?"insertBefore":"insertAfter"](g)}}else{var l=c.endNode,m=this;b();for(e=d;!e.equals(l);)g=e.getNextSourceNode(),e.type==CKEDITOR.NODE_ELEMENT&&this.checkElementRemovable(e)&&
...
a.getAttribute(f)==y(f,d[f]))b&&"data-"==f.slice(0,5)||(g=a.hasAttribute(f),a.removeAttribute(f));for(var h in c)this._.definition.fullMatch&&a.getStyle(h)!=y(h,c[h],!0)||(g=g||!!a.getStyle(h),a.removeStyle(h));u(a,e,A[a.getName()]);g&&(this._.definition.alwaysRemoveElement?r(a,1):!CKEDITOR.dtd.$block[a.getName()]||this._.enterMode==CKEDITOR.ENTER_BR&&!a.hasAttributes()?r(a):a.renameNode(this._.enterMode==CKEDITOR.ENTER_P?"p":"div"))}function v(a){for(var b=t(this),c=a.getElementsByTag(this.element),
...
var h=a(e,d);c&&g(b,h)&&(f.hide(),h=a(e,d),f.show());return!h||h.type!=CKEDITOR.NODE_ELEMENT||!h.$||G.ie&&9>G.version&&!b.boundary.equals(h)&&!b.boundary.contains(h)?null:h}}(),Z=CKEDITOR.dom.walker.whitespaces(),aa=CKEDITOR.dom.walker.nodeType(CKEDITOR.NODE_COMMENT),X=function(){function b(e){var f=e.element,h,g,k;if(!n(f)||f.contains(e.editable)||f.isReadOnly())return null;k=x(e,function(a,b){return!b.equals(a)},function(a,b){return U(a,!0,b)},f);h=k.upper;g=k.lower;if(a(e,h,g))return k.set(S,8);
if(h&&f.contains(h))for(;!h.getParent().equals(f);)h=h.getParent();else h=f.getFirst(function(a){return d(e,a)});if(g&&f.contains(g))for(;!g.getParent().equals(f);)g=g.getParent();else g=f.getLast(function(a){return d(e,a)});if(!h||!g)return null;t(e,h);t(e,g);if(!m(e.mouse.y,h.size.top,g.size.bottom))return null;for(var f=Number.MAX_VALUE,l,p,q,v;g&&!g.equals(h)&&(p=h.getNext(e.isRelevant));)l=Math.abs(c(e,h,p)-e.mouse.y),l<f&&(f=l,q=h,v=p),h=p,t(e,h);if(!q||!v||!m(e.mouse.y,q.size.top,v.size.bottom))return null;
